...
local endDate = startDate + 200	-- days
local n = 1000
local planets = table()
...
local function calcBasis(x,y,z)
	local cxx = 0
	local cxy = z.z
	local cxz = -z.y
	local cyx = -z.z
...
local function calcOrbitBasis(args, koe, planets)
	-- based on this position and velocity, find plane of orbit
	local parentBody
	local parentBodyIndex = args.parentIndex
	if parentBodyIndex == nil then
		--console.log(args.name+' has no orbit parent')
...
--]]
local function calcKOEFromPosVel(planet, planets, initJulianDate)
	...
	local gravitationalParameter = gravitationalConstant * ((planet.mass or 0) + parentBody.mass)	--m^3 / (kg s^2) * kg = m^3 / s^2
...
--		end else 
	...
	if not math.isfinite(checkPosError) then
		...
		calcOrbitBasis(planet, koe, planets)
		return
	end
...
local function updatePosVel(planetkoe, planet, planets, julianDate, initJulianDate)
	...
	planet.vel_koe = vel + planets[planet.parentIndex].vel
...
for _,planet in ipairs(planets1) do
	planet.koe = calcKOEFromPosVel(planet, planets1, julianDates[1])
end
...
for i=2,#planets do	-- skip the 1st time entry cuz thats got our KOE in it
	...
	for j=2,#planetsi do
		...
		updatePosVel(planets[1][j], planet, planetsi, julianDates[i], julianDates[1])
		...
		velErrorMag[j]:insert((planet.vel_koe - planet.vel):length() / planet.vel:length())
		--]]
	end
end
